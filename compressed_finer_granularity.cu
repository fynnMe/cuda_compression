...
__global__ void add(uint64_t *a, uint64_t *b, uint64_t *c, int num_uint64_elements) {
    ...
    for (int grid_iteration = 0;
         ...
         grid_iteration += (blockDim.x*gridDim.x)/ELEMENTS_PER_INT) {
            ...
            // Only threads that are multiples of ELEMENTS_PER_INT load data
            if (threadIdx.x % ELEMENTS_PER_INT == 0) {
                int local_uint64_index = threadIdx.x / ELEMENTS_PER_INT;
                int global_thread_number = local_uint64_index + blockIdx.x*uint64_elements_per_block + grid_iteration;
                
                if (local_uint64_index < uint64_elements_per_block) {
                    a_block[local_uint64_index] = a[global_thread_number];
                    b_block[local_uint64_index] = b[global_thread_number];
                    c_block[local_uint64_index] = 0;
                }
            }
            ...
            // Only threads that are multiples of ELEMENTS_PER_INT store results
            if (threadIdx.x % ELEMENTS_PER_INT == 0) {
                int local_uint64_index = threadIdx.x / ELEMENTS_PER_INT;
                int global_thread_number = local_uint64_index + blockIdx.x*uint64_elements_per_block + grid_iteration;
                
                if (local_uint64_index < uint64_elements_per_block) {
                    c[global_thread_number] = c_block[local_uint64_index];
                }
            }
    }
}
